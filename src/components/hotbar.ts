...
export class TelstarHotbar extends SignalWatcher(LitElement) {
  ...
  static styles = css`
    ...
    /* Compact icon grid: many channels in few rows so the player keeps the
       height. Auto-fills ~5rem cells (≈4 columns on a phone). */
    .chips {
      flex: 1;
      display: grid;
    ...
    /* Wide layout (macOS / landscape): a fixed 4-column grid so the Recent
       strip is a short 4×4 block (not a tall 2×8), leaving the viewer its
       golden-ratio share of the height. */
    ...
    /* Icon-prominent: the logo leads; the (shortened) name is the label. */
    .logo {
      width: 1.5rem;
      height: 1.1rem;
      object-fit: contain;
  ...
  render() {
    ...
    const items = order
      ...
      // Cap the Recent strip at a 4×4 block so it stays short and glanceable;
      // older recents drop off (the full list is always in the channel list).
      .slice(0, 16);
