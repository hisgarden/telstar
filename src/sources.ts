...
/** Synthetic loaded-source key for the featured FIFA+ sports pack (its own manifest). */
export const FIFA_PACK_KEY = "pack:fifa-plus";
...
export function languagePickAction(
  ...
  // A curated pack (World News or FIFA+) is never a language catalog, so a
  // language pick over it always loads that language's full playlists.
  if (loadedSourceKey === WORLD_NEWS_PACK_KEY || loadedSourceKey === FIFA_PACK_KEY) return "load";
