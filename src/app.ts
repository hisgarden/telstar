...
import { loadBaselinePack } from "./pack/baseline";
...
export class TelstarApp extends SignalWatcher(LitElement) {
  ...
  static styles = css`
    ...
    @media (max-width: 30rem) {
      ...
      /* --- Portrait stack order (operator layout) ----------------------------
         ...
         main's flex column, then order it: the two language dropdowns sit ABOVE
         the player, Refresh+clock ride at the top of the viewer (viewer-bar),
         the RECENT quick-switch sits DIRECTLY BELOW the player, and the channel
         list comes last so it claims the remaining height for scrolling. */
      .panes,
      .browse {
        display: contents;
      }
  ...
  async #loadSources(specs: LoadSpec[], force = false) {
    ...
    if (!force) {
      ...
      if (cached && cached.channels.length > 0) {
        ...
        this.#message = `${t("loaded", { count: cached.channels.length })}.`;
        this.requestUpdate();
        return;
      }
    }
  ...
  /**
   * Load the curated World News pack (baseline-only in v1; the remote-manifest
   * overlay is U4, deferred). Resolves the bundled baseline, converts to
   * channels, and routes through the same ingest → enrich → scan path as any
   * source. Enrichment fills-only, so the pack's category tags survive.
   */
  async #loadPack() {
    try {
      ...
      this.#ingestChannels(channels, WORLD_NEWS_PACK_KEY, null);
    } catch (err) {
      this.#fail(err);
    }
  }
  ...
  #ingestChannels(channels: Channel[], source: string, region: string[] | null, rejected = 0) {
    setChannels(channels);
    setLoaded(source, region);
    this.#message =
      channels.length === 0
        ? t("noPlayable")
        : `${t("loaded", { count: channels.length })}${rejected > 0 ? t("skipped", { count: rejected }) : ""}.`;
    this.requestUpdate();
  ...
  render() {
    ...
    return html`
      <main class=${fill ? "filling" : ""}>
        ...
        <div class="status">
          ${tauri
            ? html`<p class="msg" aria-live="polite">${this.#message}</p>`
            : html`<p class="hint">${t("browserHint", { cmd: "bun run tauri dev" })}</p>`}
          ${scanStatus.get() ? html`<p class="scan" aria-live="polite">${scanStatus.get()}</p>` : ""}
        ...
        <!-- One language control: load + filter by language, Refresh, and (when
             the loaded set spans countries) a region filter. -->
        <div class="controls">
          <telstar-source-picker></telstar-source-picker>
        </div>
        <!-- Viewer (player) + Browse (Recent + list): stacked in portrait,
             player as a full-height hero on the left in landscape. -->
        <div class="panes">
