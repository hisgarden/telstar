...
import { parsePack, fundingFootnoteKey, MAX_PACK_ENTRIES } from "./pack";
...
describe("fundingFootnoteKey (YouTube-style wording)", () => {
  it("Given a state-funded outlet, Then it returns the government-funded key", () => {
    expect(fundingFootnoteKey("state-funded")).toBe("fundingState");
  });

  it("Given a public broadcaster, Then it returns the publicly-funded key", () => {
    expect(fundingFootnoteKey("public-broadcaster")).toBe("fundingPublic");
  });

  it("Given commercial / newswire / unknown funding, Then no footnote (matches YouTube)", () => {
    expect(fundingFootnoteKey("commercial")).toBeNull();
    expect(fundingFootnoteKey("newswire")).toBeNull();
    expect(fundingFootnoteKey(undefined)).toBeNull();
  });
});
