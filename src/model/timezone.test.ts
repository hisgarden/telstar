...
describe("clockForCountry", () => {
  ...
  it("Given a World News pack country, When resolved, Then returns its capital + timezone", () => {
    ...
    expect(clockForCountry("tr")).toEqual({ city: "Istanbul", timeZone: "Europe/Istanbul" });
  });

  it("Given TRT World's country (tr), When resolved playing, Then Istanbul leads (broadcasterLed)", () => {
    const r = resolveClock({ playingCountry: "tr", dominantLanguage: "English", dominantCountry: "us" });
    expect(r.clock).toEqual({ city: "Istanbul", timeZone: "Europe/Istanbul" });
