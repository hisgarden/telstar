...
/**
 * The funding footnote for a channel, mirroring YouTube's publisher-funding
 * wording, or null when none applies (commercial / newswire / unknown — like
 * YouTube, which shows no panel for those). `countryName` is a human-readable
 * country name (e.g. "Japan"). The label describes funding, not editorial
 * direction (YouTube's own disclaimer applies — see docs/curation.md).
 */
export function fundingFootnote(funding: string | undefined, countryName: string): string | null {
  switch (funding) {
    case "public-broadcaster":
      return `Publicly funded broadcaster (${countryName})`;
    case "state-funded":
      return `Funded in whole or in part by the government of ${countryName}`;
    default:
      return null;
  }
}
